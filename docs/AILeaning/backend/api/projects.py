...
async def get_linvis_status(user: dict = Depends(get_current_user)):
    ...
    funny_pool = {
        ...
        "planner": {
            "low": ["正在绘制 Eino 有向拓扑图", "规划协同专家作业顺序", "整理案件执行规划大纲"],
            "medium": ["偷偷给有向图加上闭环死循环", "盘算着今天下班能提早几分钟", "把流程图的连线扭成中国结"],
            "high": ["因为有向图分支太多急得抓耳挠腮", "试图用有向图描述自己的一生", "不小心把老板的排班规划成了死锁"]
        },
        "checker": {
            "low": ["审查法条时效与废止状态", "校验草稿的诉讼请求金额", "核对判例引用合法性"],
            "medium": ["偷偷放大合规校验拦截器的阈值", "对着一份虚假证据狂翻白眼", "给小向的入库数据做体检"],
            "high": ["正因为金额超标敲警报敲到手软", "试图给法老也发一份不合规警告", "拍着桌子让不诚信的原告退庭"]
        },
        "auditor": {
            "low": ["审计最终法律起诉状", "润色定稿措辞与排版", "签署合规放行意见书"],
            "medium": ["偷偷把定稿字体换成搞笑的娃娃体", "在审计报告里藏下一首藏头诗", "试图说服小律也来一起加班"],
            "high": ["大手一挥写下：已人工修正并批准放行！", "正对着一份一塌糊涂的初稿叹气", "把终审章盖在了自己额头上"]
        }
    }
    ...
    system_status = {
        ...
        "avatar_theme": settings_data.get("avatar_theme", "xiezhi"),
...
CASE_INFO_PROMPT = """你是一名资深法律助手。请根据以下案件文档内容，提取并总结生成一份标准的案件信息摘要。

请包含以下内容（如果文档中存在相关信息）：
1. **案件名称**：案件全称
2. **案号**：法院案号
3. **案件类型**：民事/刑事/行政/知识产权等
4. **当事人信息**：
   - 原告/上诉人/申请人
   - 被告/被上诉人/被申请人
   - 第三人（如有）
5. **代理人信息**：各方律师/律所
6. **审理法院**：受理法院及审级
7. **案件事实摘要**：核心事实概述（200字以内）
8. **争议焦点**：主要争议问题
9. **诉讼请求**：原告/申请人的请求事项
10. **判决/裁定结果**：最终裁判结论（如有）
11. **关键时间线**：重要时间节点
12. **涉及法律法规**：引用的法律条文
...
async def generate_case_info(
    ...
    """
    从勾选文档中 AI 总结生成案件信息，SSE 流式输出。
    WHY: 案件信息需要从多份文档中提取关键要素，
         LLM 擅长跨文档归纳和结构化输出。
    """
    ...
    logger.info(
        f"[案件信息生成] case={case_id}, file_ids({len(req.file_ids)})={req.file_ids[:5]}"
    )

    # WHY: 不传 project_id，因为勾选的文件可能跨项目（如公共文档）
    results = query_by_file_ids(
        query_text="案件基本信息 当事人 案由 诉讼请求 事实 判决",
        file_ids=req.file_ids,
        project_id="",
        n_results=30,
    )
    logger.info(f"[案件信息生成] 检索到 {len(results)} 条结果")
    ...
    prompt = CASE_INFO_PROMPT.format(context=context) + "\n/no_think"
