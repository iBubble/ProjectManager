...
def _read_system_settings() -> dict:
    ...
    try:
        ...
        default_agents_info = {
            "chat": {"name": "小貔 (Pixiu)", "gender": "male", "avatar": "horse"},
            "service": {"name": "小律 (Lex)", "gender": "female", "avatar": "horse"},
            "legal": {"name": "法律事务专家", "gender": "male", "avatar": "horse"},
        ...
        def_val = {
            ...
            "collab_contrarian_model": settings.DEFAULT_LLM_MODEL,
            "collab_arbiter_model": settings.DEFAULT_LLM_MODEL,
            "collab_supervisor_name": "【协同】文档秘书",
            "collab_legal_name": "【协同】法律分析专家",
        ...
        for r in rows:
            k, v = r["key"], r["value"]
            if k in ("heartbeat_enabled", "eino_interrupt_enabled"):
                res[k] = v not in ("false", "0", "False")
            elif k in ("mineru_concurrency", "ragas_cron_hour"):
                try:
                    res[k] = int(v) if v else 0
                except ValueError:
                    res[k] = 0
            elif k in ("eino_checker_amount_limit", "ragas_alert_threshold"):
                try:
        ...
        if "avatar_theme" not in res:
            res["avatar_theme"] = "xiezhi"
...
async def update_system_settings(req: UpdateSystemSettingsRequest, admin: dict = Depends(require_admin)):
    ...
    if req.avatar_theme is not None:
        with get_db() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO system_settings (key, value) VALUES (?, ?)",
                ("avatar_theme", req.avatar_theme),
            )
...
async def get_public_settings():
    ...
    res = {
        "system_name": s.get("system_name", "貔貅法律知识库 V3.3.2"),
        "linvis_name": s.get("linvis_name", "麟维斯"),
        "avatar_theme": s.get("avatar_theme", "xiezhi"),
    }
    for k, v in s.items():
...
class AuditUpdateDraftRequest(BaseModel):
    trace_id: str
    draft: str
    auditor_comment: str = ""

@router.get("/audit/list")
def get_audit_list(status: str | None = None, admin: dict = Depends(require_admin)):
    """获取合规审计拦截列表，支持状态过滤"""
    from core.database import get_db
    with get_db() as conn:
        if status:
            rows = conn.execute(
                "SELECT * FROM audit_traces WHERE audit_status = ? ORDER BY id DESC",
                (status,)
            ).fetchall()
        else:
            rows = conn.execute(
                "SELECT * FROM audit_traces ORDER BY id DESC"
            ).fetchall()
    return [dict(r) for r in rows]

@router.post("/audit/approve")
def approve_audit_trace(req: AuditApproveRequest, authorization: str = Header(None), admin: dict = Depends(require_admin)):
    """批准放行合规拦截：读取冻结状态并代理向 Go 网关发起 Resume 恢复流"""
    from core.database import get_db
    import requests
    import json
    ...
    with get_db() as conn:
        row = conn.execute(
            "SELECT project_id, frozen_state FROM audit_traces WHERE trace_id = ? LIMIT 1",
            (req.trace_id,)
        ).fetchone()
    
    if not row:
        raise HTTPException(status_code=404, detail="未找到对应的拦截审计记录")
        
    project_id = row["project_id"]
    frozen_state_str = row["frozen_state"]
    if not frozen_state_str:
        raise HTTPException(status_code=400, detail="该审计记录中未包含冻结状态，无法Resume")
        
    try:
        frozen_data = json.loads(frozen_state_str)
        draft = frozen_data.get("draft", "")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"解析冻结状态失败: {e}")
        
    # 代理请求本地 Go 网关端口 :8001 进行 Resume 唤醒
    resume_url = "http://127.0.0.1:8001/api/eino/resume"
    headers = {
    ...
    try:
        # 发送恢复流请求并聚合最终答复
        r_resp = requests.post(resume_url, json=payload, headers=headers, stream=True)
        if not r_resp.ok:
            raise HTTPException(status_code=502, detail=f"向网关发起Resume失败，网关返回: {r_resp.text}")
            ...
            if line_str.startswith("data: "):
                try:
                    ...
                    if data_json.get("type") == "token":
                        final_tokens.append(data_json.get("content", ""))
        ...
        # 将结果写回 SQLite 审计表，并将状态标记为 approved
        with get_db() as conn:
            conn.execute(
                """UPDATE audit_traces 
                   SET audit_status = 'approved', auditor_comment = ?, llm_response = ? 
                   WHERE trace_id = ?""",
                (req.auditor_comment or "审批通过放行", final_answer, req.trace_id)
            )
            
        return {"status": "success", "final_answer": final_answer}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Resume 链路异常: {e}")

@router.post("/audit/reject")
def reject_audit_trace(req: AuditRejectRequest, admin: dict = Depends(require_admin)):
    """驳回/终止合规拦截流程"""
    from core.database import get_db
    with get_db() as conn:
        conn.execute(
            "UPDATE audit_traces SET audit_status = 'rejected', auditor_comment = ? WHERE trace_id = ?",
            (req.auditor_comment or "审批驳回", req.trace_id)
        )
    return {"status": "success"}

@router.post("/audit/update-draft")
def update_draft_and_approve(req: AuditUpdateDraftRequest, authorization: str = Header(None), admin: dict = Depends(require_admin)):
    """修改草案并放行：先将修正后的草案内容合并回 SQLite 冻结状态，再发起 Resume 恢复流"""
    from core.database import get_db
    import requests
    import json
    ...
    try:
        frozen_data = json.loads(frozen_state_str)
        # 将管理员修正的 draft 内容写入冻结状态中
        frozen_data["draft"] = req.draft
        new_frozen_str = json.dumps(frozen_data, ensure_ascii=False)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"解析与合并冻结状态失败: {e}")
        
    # 保存修正后的草案与合并状态至 SQLite
    with get_db() as conn:
        conn.execute(
            "UPDATE audit_traces SET frozen_state = ?, llm_response = ? WHERE trace_id = ?",
            (new_frozen_str, req.draft, req.trace_id)
        )
        ...
        with get_db() as conn:
            conn.execute(
                """UPDATE audit_traces 
                   SET audit_status = 'approved', auditor_comment = ?, llm_response = ? 
                   WHERE trace_id = ?""",
                (req.auditor_comment or "管理员修正草案后放行", final_answer, req.trace_id)
            )
