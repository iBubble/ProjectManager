...
- **后端开发**：Go (Golang 1.23+ 推荐 / 1.23.6 兼容)
...
### 1. 环境准备
- 安装 **Go (1.23 或更高版本，推荐 Go 1.23.6)** 编译后端。
...
7. **OpenAI 兼容 / 云端算力站网关支持与 8K 超长文本输出优化 (`max_tokens: 8192`)**
   - 全面支持通用 OpenAI 兼容协议 Gateway（如 DeepSeek 等云端/私有算力服务商）与本地 Ollama 私有化部署双模式。
   - 优化了请求参数配置，显式注入 `max_tokens: 8192` 与 `num_predict: 8192`（8K tokens 上限，支持单次生成 5000~6000 字），彻底解决长篇深度报告、多维度对比及合规研判被算力网关中途截断停止的问题。
   - 完善高可用容灾与离线保底机制：当远端大模型网络故障或密钥失效时，自动降级为本地规则库保底应答，确保业务连续性。
