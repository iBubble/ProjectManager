...
4. **全量项目合规研判与云南档案测评一键持久化**
   - 内置后台与 API 批处理引擎，支持对系统内全量项目完成合规健康度与云南档案 18 项指标的自动化重新研判，并持久化存盘至 `database.json` 数据库。
5. **项目阶段“自动算效推演”与“手工下拉切换”**
   - 默认根据文件在 8 大国家标准大类中的归档完备度实时推演项目生命阶段。在详情页标题旁配有下拉选择框，支持选择 `🔄 自动` 或手动指定 `立项`~`运维` 阶段。
6. **前台一站式新建项目建档 (`+ 新建项目建档`)**
   - 前台首页直接提供规范的项目建档 Modal 弹窗，支持全量字段录入，提交后自动分配 ID 并触发后台大模型研判学习。
7. **一键生成本周项目简报与 Word (.doc) 导出**
