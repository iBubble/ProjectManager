...
1. **AI 大模型运行状态监测指示灯**
   - **三色动态监测**：右上角醒目位置挂载了运行状态指示灯，页面加载时会在 1 秒内向后端发起快速握手，并实时反馈结果：
     - 🟢 **绿色**：说明已连通真实的远端大模型网关。鼠标悬浮可查看大模型品牌、端点 URL、正在使用的活动模型（如 `deepseek-r1:32b` / `qwen3.6` 等）以及连通握手网络延时。
     - 🟡 **黄色 (呼吸脉冲光晕)**：表示使用本地预设的沙箱合规规则引擎（Mock 模式），无外网时安全离线工作。
     - 🔴 **红色**：表示远端网关故障或超时，鼠标悬浮可查看具体连接错误。
2. **动态大模型网关与活动模型选择**
   - **动态网关测试**：后台管理中输入 Endpoint 和 API Key（本地私有化 Ollama 支持无密码密钥直接连通），点击即可快速握手探测。
   - **模型下拉点选**：测试通过后，后台会自动拉取网关当前加载的所有可用模型列表并生成下拉菜单，供用户自主选择保存激活模型（如 `deepseek-r1:32b`）。
3. **真实文本仿真归档公文系统**
   - **文件落盘生成**：在本地磁盘 `data/uploads/` 中为所有信息化项目一比一生成了八大阶段的仿真文本文件（包含立项建议、招标文件、采购合同、监理月报及会议纪要等），包含真实的预算、承建单位、工期和工期滞后提示等信息。
   - **真实 RAG 对话检索**：前台详情页的“项目知识库 AI 助理”可对此类仿真公文进行真实的文件内容读取与深度交叉校验，完美跑通智能 RAG 对话以及一键存为定稿公文的功能。
4. **项目生命周期总揽台账日期管理**
